...
## Supply-chain attestations (multi-arch)

On the multi-arch path the job attaches build attestations to the pushed image:

- **SLSA provenance** — `provenance: min` (default) / `max` / `false`.
- **SPDX SBOM** — `sbom: true` (default). Generated by BuildKit (`docker buildx build --attest type=sbom`) per platform. BuildKit only emits SPDX.
- **Cosign signature** — `sign: true` (default). Keyless OIDC, public images only.

### CycloneDX SBOM

BuildKit's SBOM attestation is SPDX-only. If you also need a **CycloneDX** SBOM, set `sbom-cyclonedx: true`:
