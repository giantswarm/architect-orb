...
- Two new jobs that build a multi-arch image without QEMU: `build-image-arch` builds one platform on a machine of that architecture and pushes it by digest, and `merge-image-manifests` joins the per-architecture digests into a tagged index per registry, then signs and attests it. Run one `build-image-arch` per architecture. `push-to-registries` is unchanged and remains the single-job path. See [docs/job/build-image-arch.md](docs/job/build-image-arch.md).
