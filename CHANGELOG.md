...
- SLSA provenance attestation enabled by default (`provenance: min`) on the multi-arch path. Stored as OCI 1.1 referrer artifacts where the registry supports them; falls back to inline attestation manifests otherwise. Configurable via `provenance: min|max|false`. **Note**: `max` mode embeds all `--build-arg` values verbatim into the attestation; do not use it on projects that pass secrets via `--build-arg`.
- SBOM (SPDX) attestation enabled by default (`sbom: true`) on the multi-arch path. Configurable via `sbom: true|false`.
- Cosign keyless image signing enabled by default (`sign: true`) on the multi-arch path. **Public images only** — private images are skipped at runtime to avoid leaking digests/timestamps into the public Rekor transparency log. Mints a fresh CircleCI OIDC token with `aud=sigstore` via `circleci run oidc get --claims '{"aud":"sigstore"}' --root-issuer`; the auto-injected `CIRCLE_OIDC_TOKEN_V2` is not used because its audience doesn't match what Fulcio's CircleCI federation expects.
