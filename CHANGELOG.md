...
- Cosign keyless image signing enabled by default (`sign: true`) on the multi-arch path. **Public images only** — private images are skipped at runtime to avoid leaking digests/timestamps into the public Rekor transparency log. Requires `CIRCLE_OIDC_TOKEN_V2` in the job's CircleCI context (auto-injected for context-bound jobs in OIDC-enabled orgs); the job fails loudly if the token is absent.
