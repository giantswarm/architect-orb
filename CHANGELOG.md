...
- `cache: true|false` parameter (default `false`) on `push-to-registries` enabling a buildx registry-side layer cache at `<image>:buildcache` on the first pushed registry, with `mode=max` so all intermediate layers — not just final ones — are reused on warm builds (PR rebuilds, retries on the same SHA). Off by default to avoid surprising consumers with an extra registry write per build.
