...
- `helm-chart-template`: remove `architect helm template` calls. The command now stamps `version` and `appVersion` in `Chart.yaml` directly using `gitsemver version` output, for both branch and tag builds. Helm chart validation is expected to be handled by a separate tool.
